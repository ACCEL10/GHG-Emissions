...
def connect_to_db():
    try:
        connection = mysql.connector.connect(
            host="192.168.6.1",       # Update to your MySQL server host
...
def insert_into_db(year, subcategory, fuel_type, fuel_consumed, emission_factor, emissions_kg_co2e):
    conn = connect_to_db()
    if conn is not None:
        try:
            cursor = conn.cursor()
            query = """
            INSERT INTO scope1_emissions (year, subcategory, fuel_type, fuel_consumed, emission_factor, emissions_kg_co2e)
            VALUES (%s, %s, %s, %s, %s, %s)
            """
            cursor.execute(query, (year, subcategory, fuel_type, fuel_consumed, emission_factor, emissions_kg_co2e))
            conn.commit()
...
with st.expander("Add a New Entry"):
    ...
    if st.button("Add Entry"):
        if fuel_type:
            ...
            insert_into_db(year, subcategory, fuel_type, fuel_consumed, emission_factor, emissions_kg_co2e)
